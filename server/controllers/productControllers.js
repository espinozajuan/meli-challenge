  ...
  try {
    ...
    items = items.map((item) => {
      return {
        ...
        free_shipping: item.shipping.free_shipping ? item.shipping.free_shipping : null,
        picture: item.thumbnail ? item.thumbnail : null,
        seller_state_name: item.address.state_name ? item.address.state_name : null
      }
    })
...
export const getProductInfo = async (req, res, next) => {
  ...
  try {
    let response = await fetch(`https://api.mercadolibre.com/items/${productId}`)
    const productInfo = await response.json()

    response = await fetch(`https://api.mercadolibre.com/items/${productId}/description`)
    ...
    const item = {
      ...
      free_shipping: productInfo.shipping.free_shipping ? productInfo.shipping.free_shipping : null,
      picture: productInfo.pictures[0].url ? productInfo.pictures[0].url : null,
      sold_quantity: productInfo.sold_quantity ? productInfo.sold_quantity : null,
